...
// EXACT theme configurations from Storybook login template
const themes = {
  albertHeijn: {
    name: 'Albert Heijn',
    logo: '/ah-logo.svg',
...
// EXACT LoginTemplate component from Storybook - story/page-templates-login--albert-heijn
export default function LoginPage() {
  const router = useRouter();
  const [currentTheme, setCurrentTheme] = useState('albertHeijn'); // Albert Heijn as specified
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  const theme = themes[currentTheme as keyof typeof themes] || themes.albertHeijn;
    ...
    <div className="flex h-screen">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white">
          <div>
            <Logo 
              theme={currentTheme === 'albertHeijn' ? 'albert-heijn' : 'gambit'}
              className="mb-8"
            />
          </div>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="">
                      <Logo 
                        theme={currentTheme === 'albertHeijn' ? 'albert-heijn' : 'gambit'}
                        variant={currentTheme === 'albertHeijn' ? 'blue' : 'original'}
                      />
                    </button>
                  </DropdownMenuTrigger>
                    ...
                    {Object.entries(themes).map(([key, themeOption]) => (
                      ...
                      >
                        <div className="w-5 h-5 flex items-center justify-center">
                          <Logo 
                            theme={key === 'albertHeijn' ? 'albert-heijn' : 'gambit'}
                            variant={key === 'albertHeijn' ? 'blue' : 'original'}
                            className="w-5 h-5"
                          />
                        </div>
