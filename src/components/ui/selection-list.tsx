    ...
    return (
      <div ref={ref} className={cn("space-y-1", className)}>
        {items.map((item) => (
          <div
            key={item.id}
            className="flex items-center gap-3 px-3 py-2.5 rounded-md border border-surface-selected-border bg-surface-selected hover:bg-surface-hover transition-colors"
          >
            {variant === "switch" && (
              <Switch
                checked={true}
